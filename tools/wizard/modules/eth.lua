...
arch.stm32f1.configure = function()
        local function configure_priority()
                local choice = key_read(db.path.stm32f1.eth.flags, "__ETH_IRQ_PRIORITY__")
                ...
                choice = get_selection()
                if can_be_saved(choice) then
                        key_save(db.path.stm32f1.eth.flags, "__ETH_IRQ_PRIORITY__", choice)
                end
        ...
        local function configure_hw_checksum()
                local choice = key_read(db.path.stm32f1.eth.flags, "__ETH_CHECKSUM_BY_HARDWARE__")
                ...
                choice = get_selection()
                if (can_be_saved(choice)) then
                        key_save(db.path.stm32f1.eth.flags, "__ETH_CHECKSUM_BY_HARDWARE__", choice)
                end
        ...
        local function configure_speed()
                ...
                local choice = key_read(db.path.stm32f1.eth.flags, "__ETH_SPEED__")
                ...
                choice = get_selection()
                if (can_be_saved(choice)) then
                        key_save(db.path.stm32f1.eth.flags, "__ETH_SPEED__", choice)
                end
        ...
        local function configure_PHY_address()
                local choice = key_read(db.path.stm32f1.eth.flags, "__ETH_PHY_ADDRESS__")
                msg(progress() .. "PHY address configuration.")
                msg("Current choice is: " .. choice .. ".")
                choice = get_number("hex", 0x00, 0xFF)
                if (can_be_saved(choice)) then
                        key_save(db.path.stm32f1.eth.flags, "__ETH_PHY_ADDRESS__", choice)
                end
        ...
        local function configure_reset_delay()
                local choice = key_read(db.path.stm32f1.eth.flags, "__ETH_PHY_RESET_DELAY__")
                msg(progress() .. "PHY reset delay configuration.")
                msg("Current choice is: " .. choice .. " ms.")
                choice = get_number("dec", 0, 500)
                if (can_be_saved(choice)) then
                        key_save(db.path.stm32f1.eth.flags, "__ETH_PHY_RESET_DELAY__", choice)
                end
        ...
        local function configure_config_delay()
                local choice = key_read(db.path.stm32f1.eth.flags, "__ETH_PHY_CONFIG_DELAY__")
                msg(progress() .. "PHY setup delay configuration.")
                msg("Current choice is: " .. choice .. " ms.")
                choice = get_number("dec", 0, 500)
                if (can_be_saved(choice)) then
                        key_save(db.path.stm32f1.eth.flags, "__ETH_PHY_CONFIG_DELAY__", choice)
                end
        ...
        local function configure_PHY_device()
                ...
                local choice = key_read(db.path.stm32f1.eth.flags, "__ETH_DEVICE__")
                ...
                if (can_be_saved(choice)) then
                        ...
                        key_save(db.path.stm32f1.eth.flags, "__ETH_DEVICE__", choice)
                        key_save(db.path.stm32f1.eth.flags, "__ETH_PHY_SR__", status_reg_address)
                        key_save(db.path.stm32f1.eth.flags, "__ETH_PHY_SPEED_STATUS_BM__", speed_status_bm)
                        key_save(db.path.stm32f1.eth.flags, "__ETH_PHY_DUPLEX_STATUS_BM__", duplex_status_bm)
                end
        ...
        local function print_summary()
                ...
                local priority = key_read(db.path.stm32f1.eth.flags, "__ETH_IRQ_PRIORITY__")
                local checksum = key_read(db.path.stm32f1.eth.flags, "__ETH_CHECKSUM_BY_HARDWARE__")
                local ethspeed = key_read(db.path.stm32f1.eth.flags, "__ETH_SPEED__")
                local phyaddr  = key_read(db.path.stm32f1.eth.flags, "__ETH_PHY_ADDRESS__")
                local resetdly = key_read(db.path.stm32f1.eth.flags, "__ETH_PHY_RESET_DELAY__")
                local setupdly = key_read(db.path.stm32f1.eth.flags, "__ETH_PHY_CONFIG_DELAY__")
                local ethdev   = key_read(db.path.stm32f1.eth.flags, "__ETH_DEVICE__")
...
function eth:configure()
        title("ETH module configuration for " .. db:get_cpu_name())

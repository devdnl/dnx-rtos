...
arch.stm32f1.configure = function()
        ...
        local function calc_number_of_steps(spi)
                local spi_en   = key_read(db.path.stm32f1.spi.flags, "__SPI_SPI"..spi.."_ENABLE__")
                local no_of_cs = key_read(db.path.stm32f1.spi.flags, "__SPI_SPI"..spi.."_NUMBER_OF_CS__")
        ...
        local function configure_dummy_byte()
                local choice = key_read(db.path.stm32f1.spi.flags, "__SPI_DEFAULT_DUMMY_BYTE__")
                ...
                if (can_be_saved(choice)) then
                        key_save(db.path.stm32f1.spi.flags, "__SPI_DEFAULT_DUMMY_BYTE__", choice)
                end
        ...
        local function configure_clk_divider()
                local choice = key_read(db.path.stm32f1.spi.flags, "__SPI_DEFAULT_CLK_DIV__")
                ...
                for i = 1, 8 do
                        add_item("SPI_CLK_DIV_"..2^i, "/"..2^i)
                end
                choice = get_selection()
                if (can_be_saved(choice)) then
                        key_save(db.path.stm32f1.spi.flags, "__SPI_DEFAULT_CLK_DIV__", choice)
                end
        ...
        local function configure_mode()
                ...
                local choice = key_read(db.path.stm32f1.spi.flags, "__SPI_DEFAULT_MODE__")
                ...
                choice = get_selection()
                if (can_be_saved(choice)) then
                        key_save(db.path.stm32f1.spi.flags, "__SPI_DEFAULT_MODE__", choice)
                end
        ...
        local function configure_msb_first()
                ...
                local choice = key_read(db.path.stm32f1.spi.flags, "__SPI_DEFAULT_MSB_FIRST__")
                ...
                choice = get_selection()
                if can_be_saved(choice) then
                        key_save(db.path.stm32f1.spi.flags, "__SPI_DEFAULT_MSB_FIRST__", choice)
                end
        ...
        local function configure_spi_if_enable(spi)
                ...
                local choice = key_read(db.path.stm32f1.spi.flags, key)
                ...
                choice = get_selection()
                if can_be_saved(choice) then
                        key_save(db.path.stm32f1.spi.flags, key, choice)
                end
        ...
        local function spi_select()
                local spi_no = db:get_mcu_module_data(db:get_cpu_name(), "SPI")[1]
                ...
                while true do
                        ...
                        if key_read(db.path.stm32f1.spi.flags, "__SPI_SPI"..choice.."_ENABLE__") == yes then
        ...
        local function print_summary()
                local str
                local dummy_byte = key_read(db.path.stm32f1.spi.flags, "__SPI_DEFAULT_DUMMY_BYTE__")
                local clk_div    = key_read(db.path.stm32f1.spi.flags, "__SPI_DEFAULT_CLK_DIV__")
                local mode       = key_read(db.path.stm32f1.spi.flags, "__SPI_DEFAULT_MODE__")
                local msb_first  = key_read(db.path.stm32f1.spi.flags, "__SPI_DEFAULT_MSB_FIRST__")
                local spi_no     = db:get_mcu_module_data(db:get_cpu_name(), "SPI")[1]
                ...
                for spi = 1, spi_no do
                        local spi_en   = key_read(db.path.stm32f1.spi.flags, "__SPI_SPI"..spi.."_ENABLE__")
                        local spi_prio = key_read(db.path.stm32f1.spi.flags, "__SPI_SPI"..spi.."_PRIORITY__")
                        local no_of_cs = key_read(db.path.stm32f1.spi.flags, "__SPI_SPI"..spi.."_NUMBER_OF_CS__")
                        ...
                        if spi_en == yes then
                                ...
                                for cs = 0, no_of_cs - 1 do
                                        local cs_name = key_read(db.path.stm32f1.spi.flags, "__SPI_SPI"..spi.."_CS"..cs.."_PIN_NAME__")
...
function spi:configure()
        title("SPI module configuration for " .. db:get_cpu_name())
        navigation("Home/Modules/SPI")
        calculate_total_steps()

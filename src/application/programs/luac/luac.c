...
@author  See Copyright Notice in lua.h

@brief   Lua compiler

@note    Copyright (C) 2015 See Copyright Notice in lua.h <See Copyright Notice in lua.h>
...
static int doargs(int argc, char* argv[])
{
        ...
        for (i = 1; i < argc; i++) {
                ...
                else if (IS("-o")) /* output file */
                {
                        ...
                        || *global->output == 0
                        || (*global->output == '-' && global->output[1] != 0))
...
static int pmain(lua_State* L)
{
        ...
        f = combine(L, argc);
        if (global->listing)
                luaU_print(f, global->listing > 1);
        if (global->dumping) {
                FILE* D =
                (global->output == NULL) ? stdout : fopen(global->output, "wb");
                if (D == NULL)
                        cannot("open");
                lua_lock(L);
...
int_main(luac, STACK_DEPTH_LARGE, int argc, char* argv[])
{

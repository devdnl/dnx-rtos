...
GLOBAL_VARIABLES_SECTION {
        ...
        const char* output;            /* actual output file name */
        const char* progname;          /* actual program name */
        TString **tmname;
};
PROGRAM_PARAMS(luac, STACK_DEPTH_MEDIUM);
...
{
 fprintf(stderr,"%s: cannot %s %s: %s\n",global->progname,what,global->output,strerror(errno));
 exit(EXIT_FAILURE);
}
...
static int doargs(int argc, char* argv[])
{
 int i;
 int version=0;
 if (argv[0]!=NULL && *argv[0]!=0) global->progname=argv[0];
 for (i=1; i<argc; i++)
 {
  ...
  {
   global->output=argv[++i];
   if (global->output==NULL || *global->output==0 || (*global->output=='-' && global->output[1]!=0))
    usage("'-o' needs argument");
   if (IS("-")) global->output=NULL;
  }
 ...
 if (i==argc && (global->listing || !global->dumping))
 {
  global->dumping=0;
  argv[--i]=(char*)Output;
 }
...
static int pmain(lua_State* L)
{
 ...
 f=combine(L,argc);
 if (global->listing) luaU_print(f,global->listing>1);
 if (global->dumping)
 {
  FILE* D= (global->output==NULL) ? stdout : fopen(global->output,"wb");
  if (D==NULL) cannot("open");
  lua_lock(L);
  luaU_dump(L,f,writer,D,global->stripping);
  lua_unlock(L);
...
{
 global->listing=0;                   /* list bytecodes? */
 global->dumping=1;                   /* dump bytecodes? */
 global->stripping=0;                 /* strip debug information? */
 global->output=Output;               /* actual output file name */
 global->progname=PROGNAME;           /* actual program name */
...
#define eventname(i) (getstr(global->tmname[i]))

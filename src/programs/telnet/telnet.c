                ...
                if (err == NET_ERR_OK) {
                        ...
                        if (net_buf_data(rx_buffer, (void *)&string, &len) == NET_ERR_OK) {
                                ...
                                if (strncmp(string, "test\r\n", len) == 0) {
                                        ...
                                        net_write(conn, str, strlen(str), NET_CONN_FLAG_NOCOPY);

                                        char *test = calloc(1, 50);
                                        strcpy(test, "Test buffer...\n");
                                        net_write(conn, test, strlen(test), NET_CONN_FLAG_COPY);
                                        free(test);
...
PROGRAM_MAIN(telnet, int argc, char *argv[])
{
        ...
        net_conn_t *listener = net_new_conn(NET_CONN_TYPE_TCP);
        if (!listener) {
                puts("Connection failed");
                goto exit;
        }

        if (net_bind(listener, (net_ip_t *)&ip_addr_any, 23) != NET_ERR_OK) {
                puts("Bind failed");
                goto exit;
        }

        if (net_listen(listener) != NET_ERR_OK) {
                puts("Listen error");
                goto exit;
        }
        ...
        for (;;) {
                net_conn_t *new_conn ;
                if (net_accept(listener, &new_conn) != NET_ERR_OK) {

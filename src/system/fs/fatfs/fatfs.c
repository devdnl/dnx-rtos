...
API_FS_OPEN(fatfs, void *fs_handle, void **extra, fd_t *fd, fpos_t *fpos, const char *path, u32_t flags)
{
        ...
        *extra = fat_file;
        ...
        int result = faterr_2_errno(libfat_open(&hdl->fatfs, fat_file, path, fat_mode));
        if (result != ESUCC) {
                free(fat_file);
                return result;
        }
...
API_FS_CLOSE(fatfs, void *fs_handle, void *extra, fd_t fd, bool force)
{
        ...
        int result = faterr_2_errno(libfat_close(fat_file));
        if (result == ESUCC) {
                free(fat_file);
                hdl->opened_files--;
        }
             ...
             struct vfs_fattr fattr)
{
        ...
        if (libfat_tell(fat_file) != (u32_t)*fpos) {
                result = faterr_2_errno(libfat_lseek(fat_file, (u32_t)*fpos));
                if (result != ESUCC) {
                        return result;
                }
        }
        ...
        uint n = 0;
        result = faterr_2_errno(libfat_write(fat_file, src, count, &n));
        if (result == ESUCC) {
                *wrcnt = n;
        }
            ...
            struct vfs_fattr fattr)
{
        ...
        uint n = 0;
        result = faterr_2_errno(libfat_read(fat_file, dst, count, &n));
        if (result == ESUCC) {
                *rdcnt = n;
        }
...
API_FS_OPENDIR(fatfs, void *fs_handle, const char *path, DIR *dir)
{
        ...
        int result = faterr_2_errno(libfat_opendir(&hdl->fatfs, &fatdir->dir, dospath));
        if (result == ESUCC) {
                free(dospath);
                hdl->opened_dirs++;
        } else {

              ...
              struct vfs_fattr  fattr)
{
        ...
        if (hdl->sector_count == 0) {
                printk("NVM: write at not configured device");
                return EINVAL;
        }

        if (*fpos >= hdl->total_size) {
                printk("NVM: write pointer out of range");
                return ENOSPC;
        }
             ...
             struct vfs_fattr fattr)
{
        ...
        if (hdl->sector_count == 0) {
                printk("NVM: read at not configured device");
                return EINVAL;
        }

        if (*fpos >= hdl->total_size) {
                printk("NVM: read pointer out of range");
                return ENOSPC;
        }
        ...
        while (not sys_time_is_expired(tref, sys_get_uptime_ms())) {
                ...
                if (FLASH->SR & FLASH_SR_BSY) {
                        ...
                        if (FLASH->SR & FLASH_SR_WRPERR) {
                                printk("NVM: write protection error");
                                err = EIO;
                                break;

                        } else if (FLASH->SR & 0xEF) {
                                printk("NVM: program error");
                                err = EIO;
                                break;

                        } else if (FLASH->SR & FLASH_SR_SOP) {
                                printk("NVM: operation error");
                                err = EIO;
                                break;
                        } else {
                                err = ESUCC;

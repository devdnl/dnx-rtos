...
void ttyedit_set_value(ttyedit_t *this, const char *str, bool show)
{
        if (is_valid(this) && str) {
                if (strlen(str) <= EDITLINE_LEN) {
                        ...
                        if (show) {
                                _vfs_fwrite(str, sizeof(char), strlen(str), this->out_file);

                                static const char *erase_line_end = ERASE_LINE_END;
                                _vfs_fwrite(erase_line_end, sizeof(char), strlen(erase_line_end), this->out_file);
                        }
                }
        }
}
        ...
        if (is_valid(this)) {
                ...
                if (this->cursor_position < this->length) {
                        ...
                        if (this->echo_enabled) {
                                ...
                                _vfs_fwrite(cmd, sizeof(char), strlen(cmd), this->out_file);

                                cmd = &this->buffer[this->cursor_position - 1];
                                _vfs_fwrite(cmd, sizeof(char), this->length - (this->cursor_position - 1), this->out_file);
                        ...
                        if (this->echo_enabled) {
                                _vfs_fwrite(&c, sizeof(char), 1, this->out_file);
                        }
                }
        }
}
...
void ttyedit_remove_char(ttyedit_t *this)
{
        if (is_valid(this)) {
                ...
                _vfs_fwrite(cmd, sizeof(char), this->length - this->cursor_position, this->out_file);
...
void ttyedit_move_cursor_left(ttyedit_t *this)
{
        if (is_valid(this)) {
                if (this->cursor_position > 0) {
                        _vfs_fwrite("\b", sizeof(char), 1, this->out_file);
                        this->cursor_position--;
                }
        }
}

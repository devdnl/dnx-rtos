                              ...
                              uint8_t id), uint8_t id)
{
  ...
  /* Create USB Host Queue */
   if (sys_queue_create(MSGQUEUE_OBJECTS, sizeof(phost->os_msg), &phost->os_event) != 0)
   {
     return USBH_FAIL;
   }
  ...
  phost->thread_run = true;

  if (sys_thread_create(USBH_Process_OS, &USB_STACK_THREAD_ATTR, phost, &phost->thread) != 0)
  {
     sys_queue_destroy(phost->os_event);
     return USBH_FAIL;
  }
...
static USBH_StatusTypeDef USBH_HandleEnum(USBH_HandleTypeDef *phost)
{
  ...
  switch (phost->EnumState)
  {
    ...
    case ENUM_GET_MFC_STRING_DESC:
      strcpy(phost->manufacturer_name, "N/A");
    ...
    case ENUM_GET_SERIALNUM_STRING_DESC:
      strcpy(phost->serial_number, "N/A");

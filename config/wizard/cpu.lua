...
uc.stm32f1.mcu                  = {}
uc.stm32f1.mcu.STM32F100C4xx    = {["RAM"] = 4096 , ["family"] = "STM32F10X_LD_VL", ["modules"] = {"GPIO", "PLL", "SDSPI", "SPI", "TTY", "UART", "WDG", "I2S", "I2C", "ADC", "DAC", "HDMICEC"}, ["GPIO"] = 4, ["SPI"] = 1, ["UART"] = 2}
...
local function configure_mcu_name()
        ...
        if (can_be_saved(choice)) then
                ...
                key_save(dir.."/cpu.h", "__CPU_FAMILY__", "__" .. uc[arch].mcu[choice].family .. "__")
                key_save(dir.."/cpu.h", "__CPU_RAM_MEM_SIZE__", uc[arch].mcu[choice].RAM)
...
--------------------------------------------------------------------------------
-- @brief Function return number of GPIO ports
-- @return number of GPIO ports
--------------------------------------------------------------------------------
function cpu:get_number_of_GPIO_ports()
        return uc[cpu:get_arch()].mcu[cpu:get_name()].GPIO
end

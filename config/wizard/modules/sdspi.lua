...
arch.stm32f1.configure = function()
        ...
        local function configure_card_timeout()
                local choice = key_read("../stm32f1/sdspi_flags.h", "__SDSPI_TIMEOUT__")
                msg(progress() .. "Card timeout configuration.")
                msg("Current choice is: " .. choice .. " ms.")
                choice = get_number("dec", 0, 1000)
                if (can_be_saved(choice)) then
                        key_save("../stm32f1/sdspi_flags.h", "__SDSPI_TIMEOUT__", choice)
                end
        ...
        local function configure_SPI_port()
                local choice = key_read("../stm32f1/sdspi_flags.h", "__SDSPI_SPI_PORT__")
                ...
                for i = 1, db:get_mcu_module_data(cpu:get_name(), "SPI")[1] do
                        add_item(i, "SPI"..i)
                end
                choice = get_selection()
                if (can_be_saved(choice)) then
                        key_save("../stm32f1/sdspi_flags.h", "__SDSPI_SPI_PORT__", choice)
                end
        ...
        local function configure_SPI_clock_prescaler()
                local choice = key_read("../stm32f1/sdspi_flags.h", "__SDSPI_SPI_CLK_DIV__")
                ...
                choice = get_selection()
                if (can_be_saved(choice)) then
                        key_save("../stm32f1/sdspi_flags.h", "__SDSPI_SPI_CLK_DIV__", choice)
                end
        ...
        local function configure_card_CS_pin()
                local choice = key_read("../stm32f1/sdspi_flags.h", "__SDSPI_SD_CS_PIN__")
                ...
                choice = get_selection()
                if can_be_saved(choice) then
                        key_save("../stm32f1/sdspi_flags.h", "__SDSPI_SD_CS_PIN__", choice)
                end
        ...
        local function configure_enable_DMA()
                local choice = key_read("../stm32f1/sdspi_flags.h", "__SDSPI_ENABLE_DMA__")
                ...
                choice = get_selection()
                if can_be_saved(choice) then
                        key_save("../stm32f1/sdspi_flags.h", "__SDSPI_ENABLE_DMA__", choice)
                end
        ...
        local function configure_DMA_IRQ_priority()
                local choice = key_read("../stm32f1/sdspi_flags.h", "__SDSPI_DMA_IRQ_PRIORITY__")
                ...
                choice = get_selection()
                if can_be_saved(choice) then
                        key_save("../stm32f1/sdspi_flags.h", "__SDSPI_DMA_IRQ_PRIORITY__", choice)
                end
        ...
        local function print_summary()
                local timeout  = key_read("../stm32f1/sdspi_flags.h", "__SDSPI_TIMEOUT__")
                local spi_port = key_read("../stm32f1/sdspi_flags.h", "__SDSPI_SPI_PORT__")
                local spi_div  = key_read("../stm32f1/sdspi_flags.h", "__SDSPI_SPI_CLK_DIV__")
                local cs_pin   = key_read("../stm32f1/sdspi_flags.h", "__SDSPI_SD_CS_PIN__")
                local dma_en   = key_read("../stm32f1/sdspi_flags.h", "__SDSPI_ENABLE_DMA__")
                local dma_prio = key_read("../stm32f1/sdspi_flags.h", "__SDSPI_DMA_IRQ_PRIORITY__")

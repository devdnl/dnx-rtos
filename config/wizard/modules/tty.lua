...
local function ask_for_enable()
        local choice = key_read("../project/flags.h", "__ENABLE_TTY__")
        ...
        if can_be_saved(choice) then
                key_save("../project/flags.h", "__ENABLE_TTY__", choice)
                key_save("../project/Makefile", "ENABLE_TTY", choice)
        end
...
local function configure_tty()
        ...
        local function configure_term_cols()
                local choice = key_read("../stm32f1/tty_flags.h", "__TTY_TERM_COLS__")
                ...
                if (can_be_saved(choice)) then
                        key_save("../stm32f1/tty_flags.h", "__TTY_TERM_COLS__", choice)
                end
        ...
        local function configure_term_rows()
                local choice = key_read("../stm32f1/tty_flags.h", "__TTY_TERM_ROWS__")
                ...
                if (can_be_saved(choice)) then
                        key_save("../stm32f1/tty_flags.h", "__TTY_TERM_ROWS__", choice)
                end
        ...
        local function configure_out_stream_len()
                local choice = key_read("../stm32f1/tty_flags.h", "__TTY_OUT_STREAM_LEN__")
                ...
                if (can_be_saved(choice)) then
                        key_save("../stm32f1/tty_flags.h", "__TTY_OUT_STREAM_LEN__", choice)
                end
        ...
        local function configure_number_of_terminals()
                local choice = key_read("../stm32f1/tty_flags.h", "__TTY_NUMBER_OF_TERM__")
                ...
                choice = get_selection()
                if (can_be_saved(choice)) then
                        key_save("../stm32f1/tty_flags.h", "__TTY_NUMBER_OF_TERM__", choice)
                end
        ...
        local function configure_term_size_check_enable()
                local choice = key_read("../stm32f1/tty_flags.h", "__TTY_ENABLE_TERM_SIZE_CHECK__")
                ...
                choice = get_selection()
                if (can_be_saved(choice)) then
                        key_save("../stm32f1/tty_flags.h", "__TTY_ENABLE_TERM_SIZE_CHECK__", choice)
                end
        ...
        local function configure_term_in_file()
                local choice = key_read("../stm32f1/tty_flags.h", "__TTY_TERM_IN_FILE__")
                ...
                choice = get_string()
                if (can_be_saved(choice)) then
                        key_save("../stm32f1/tty_flags.h", "__TTY_TERM_IN_FILE__", choice)
                end
        ...
        local function configure_term_out_file()
                local choice = key_read("../stm32f1/tty_flags.h", "__TTY_TERM_OUT_FILE__")
                msg(progress() .. "Enter terminal output file. File must support multithreading. "..
                                  "Both output and input files, can be the same file; typically it is an UART device file.")
                msg("Current file is: " .. choice .. ".")
                choice = get_string()
                if (can_be_saved(choice)) then
                        key_save("../stm32f1/tty_flags.h", "__TTY_TERM_OUT_FILE__", choice)
                end
        ...
        local function print_summary()
                local cols            = key_read("../stm32f1/tty_flags.h", "__TTY_TERM_COLS__")
                local rows            = key_read("../stm32f1/tty_flags.h", "__TTY_TERM_ROWS__")
                local out_stream_len  = key_read("../stm32f1/tty_flags.h", "__TTY_OUT_STREAM_LEN__")
                local no_of_term      = key_read("../stm32f1/tty_flags.h", "__TTY_NUMBER_OF_TERM__")
                local term_size_check = key_read("../stm32f1/tty_flags.h", "__TTY_ENABLE_TERM_SIZE_CHECK__")
                local in_file         = key_read("../stm32f1/tty_flags.h", "__TTY_TERM_IN_FILE__")
                local out_file        = key_read("../stm32f1/tty_flags.h", "__TTY_TERM_OUT_FILE__")
        ...
        if key_read("../project/Makefile", "ENABLE_TTY") == yes then

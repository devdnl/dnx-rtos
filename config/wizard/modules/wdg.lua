...
local function ask_for_enable()
        local choice = key_read("../project/flags.h", "__ENABLE_WDG__")
        ...
        if can_be_saved(choice) then
                key_save("../project/flags.h", "__ENABLE_WDG__", choice)
                key_save("../project/Makefile", "ENABLE_WDG", choice)
        end
...
arch.stm32f1.configure = function()
        ...
        local function configure_dev_lock_at_open()
                local choice = key_read("../stm32f1/wdg_flags.h", "__WDG_DEVICE_LOCK_AT_OPEN__")
                ...
                choice = get_selection()
                if can_be_saved(choice) then
                        key_save("../stm32f1/wdg_flags.h", "__WDG_DEVICE_LOCK_AT_OPEN__", choice)
                end
        ...
        local function configure_wdg_timeout()
                ...
                local divider = key_read("../stm32f1/wdg_flags.h", "__WDG_CLK_DIVIDER__")
                local reload  = key_read("../stm32f1/wdg_flags.h", "__WDG_RELOAD_VALUE__")
                ...
                if can_be_saved(choice) then
                        ...
                        key_save("../stm32f1/wdg_flags.h", "__WDG_CLK_DIVIDER__",  divtab[choice][2])
                        key_save("../stm32f1/wdg_flags.h", "__WDG_RELOAD_VALUE__", divtab[choice][3])
                end
        ...
        local function configure_behaviour_on_debug()
                local choice = key_read("../stm32f1/wdg_flags.h", "__WDG_DISABLE_ON_DEBUG__")
                ...
                choice = get_selection()
                if can_be_saved(choice) then
                        key_save("../stm32f1/wdg_flags.h", "__WDG_DISABLE_ON_DEBUG__", choice)
                end
        ...
        local function print_summary()
                local dev_lock = key_read("../stm32f1/wdg_flags.h", "__WDG_DEVICE_LOCK_AT_OPEN__")
                local divider  = key_read("../stm32f1/wdg_flags.h", "__WDG_CLK_DIVIDER__")
                local reload   = key_read("../stm32f1/wdg_flags.h", "__WDG_RELOAD_VALUE__")
                local debug    = key_read("../stm32f1/wdg_flags.h", "__WDG_DISABLE_ON_DEBUG__")
                local lsi      = pll:get_frequencies().LSI
        ...
        if key_read("../project/Makefile", "ENABLE_WDG") == yes then
...
function wdg:configure()
        title("WDG module configuration for " .. cpu:get_name())

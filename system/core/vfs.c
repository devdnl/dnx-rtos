...
//static ch_t          *AddEndSlash(const ch_t *path);/* DNLTEST*/
//static ch_t          *SubEndSlash(const ch_t *path);/* DNLTEST*/
...
stdRet_t vfs_mount(const ch_t *srcPath, const ch_t *mntPoint, struct vfs_fscfg *mountcfg)
{
        ...
        ch_t          *newpath = NewCorrectedPath(mntPoint, ADD_SLASH);// AddEndSlash(mntPoint); /* DNLTEST*/

        /* DNLFIXME correct this code a little bit */
        if (newpath) {
...
stdRet_t vfs_umount(const ch_t *path)
{
        ...
        u32_t  itemid;
        ch_t  *newpath = NewCorrectedPath(path, ADD_SLASH); //AddEndSlash(path); /* DNLTEST*/
...
stdRet_t vfs_mknod(const ch_t *path, struct vfs_drvcfg *drvcfg)
{
        ...
        if (path && drvcfg && vfs) {
                ...
                if (fs) {
                        if (fs->fs.f_mknod) {
                                status = fs->fs.f_mknod(fs->fs.f_fsd,
                                                        extPath,
                                                        drvcfg);
                        }
                }
...
stdRet_t vfs_mkdir(const ch_t *path)
{
        ...
        if (path && vfs) {
                ch_t *newpath = NewCorrectedPath(path, SUB_SLASH);//SubEndSlash(path);/* DNLTEST*/
                ...
                if (newpath) {
                        ...
                        if (fs) {
                                if (fs->fs.f_mkdir) {
                                        status = fs->fs.f_mkdir(fs->fs.f_fsd,
                                                                extPath);
                                }
                        }
...
stdRet_t vfs_remove(const ch_t *path)
{
        ...
        if (path && vfs) {
                ch_t *slashpath = NewCorrectedPath(path, ADD_SLASH);//AddEndSlash(path);/* DNLTEST*/
                ...
                if (slashpath) {
                        ...
                        if (bfs && mfs == NULL) {
                                if (bfs->fs.f_remove) {
                                        status = bfs->fs.f_remove(bfs->fs.f_fsd,
                                                                  extPath);
                                }
                        }
        ...
        if (oldName && newName && vfs) {
                ...
                if (fsOld && fsNew && fsOld == fsNew) {
                        if (fsOld->fs.f_rename) {
                                status = fsOld->fs.f_rename(fsOld->fs.f_fsd,
                                                            extPathOld,
                                                            extPathNew);
                        }
                }
...
stdRet_t vfs_chown(const ch_t *path, u16_t owner, u16_t group)
{
        ...
        if (path && vfs) {
                ...
                if (fs) {
                        if (fs->fs.f_chown) {
                                status = fs->fs.f_chown(fs->fs.f_fsd,
                                                        extPath,
                                                        owner,
                                                        group);
                        }
...
stdRet_t vfs_stat(const ch_t *path, struct vfs_stat *stat)
{
        ...
        if (path && stat && vfs) {
                ...
                if (fs) {
                        if (fs->fs.f_stat) {
                                status = fs->fs.f_stat(fs->fs.f_fsd,
                                                       extPath,
                                                       stat);
                        }
                }
...
stdRet_t vfs_statfs(const ch_t *path, struct vfs_statfs *statfs)
{
        ...
        if (path && statfs && vfs) {
                ...
                if (slashpath) {
                        ...
                        if (fs) {
                                if (fs->fs.f_statfs) {
                                        status = fs->fs.f_statfs(fs->fs.f_fsd,
                                                                 statfs);
                                }
                        }
           ...
           && (  strncmp("r", mode, 2) == 0 || strncmp("r+", mode, 2) == 0
              ...
              || strncmp("a", mode, 2) == 0 || strncmp("a+", mode, 2) == 0) ) {
                ...
                if (file) {
                        ...
                        if (fs) {
                                if (fs->fs.f_open) {
                                        status = fs->fs.f_open(fs->fs.f_fsd,
                                                               &file->fd,
                                                               &file->f_seek,
                                                               extPath, mode);
                                }
...
static ch_t *NewCorrectedPath(const ch_t *path, enum pathCorrection corr)
{
        ...
        if (corr != ADD_SLASH || corr != SUB_SLASH) {
                return NULL;
        }
